...
class FantasyMatchupViewModel: ObservableObject {
   ...
   func getRoster(for matchup: AnyFantasyMatchup, teamIndex: Int) -> [FantasyScores.FantasyModel.Team.PlayerEntry] {
	  ...
	  let desiredOrder: [Int] = [0, 2, 3, 4, 5, 6, 23, 16, 17]
	  let slotOrder = Dictionary(uniqueKeysWithValues: desiredOrder.enumerated().map { ($1, $0) })

	  return team.roster?.entries
		 .filter { player in
			// Filter active roster players (by position slots specified in the order)
			desiredOrder.contains(player.lineupSlotId)
		 }
		 .sorted {
			// Sort players according to desiredOrder based on lineupSlotId
