...
extension FantasyMatchupViewModel {
   ...
   func fetchFantasyData(forWeek week: Int) {
	  ...
	  // Restore original views that were working
	  ...
	  URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
		 ...
		 if let data = data {
			do {
			   ...
			   print("DP - ESPN Team Stats:")
			   if let teams = json?["teams"] as? [[String: Any]] {
				  for team in teams {
					 if let teamId = team["id"] as? Int,
						let settings = team["settings"] as? [String: Any] {
						print("DP - Team ID: \(teamId)")
						print("DP - Points For: \(settings["points"] ?? "N/A")")
						print("DP - Points Against: \(settings["pointsAgainst"] ?? "N/A")")
						print("DP - Rank: \(settings["rank"] ?? "N/A")")
					 }
				  }
			   }
			   
			   // Keep original decoding logic
			   let model = try JSONDecoder().decode(FantasyScores.FantasyModel.self, from: data)
   ...
   func processESPNMatchups(model: FantasyScores.FantasyModel) {
	  ...
	  for matchup in model.schedule.filter({ $0.matchupPeriodId == selectedWeek }) {
		 fetchESPNRecords(forWeek: selectedWeek) // added to get records W-L-T
